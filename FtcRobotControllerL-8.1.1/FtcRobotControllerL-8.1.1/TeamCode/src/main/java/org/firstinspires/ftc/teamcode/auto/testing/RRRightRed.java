...
import org.firstinspires.ftc.teamcode.auto.OpenCV.AprilTagAutonomousInitDetectionExample;

import java.util.ArrayList;

@Autonomous(name = "RightRED")
public class RRRightRed  extends LinearOpMode{
    ...
    public void runOpMode() throws InterruptedException {
        ...
        drive.setPoseEstimate(startPose);
                    //preloaded cone
        Trajectory pLoadRedMid = drive.trajectoryBuilder(startPose)
                .forward(20)
                .splineTo(new Vector2d(PreLoadMiddleHigh.getX(), PreLoadMiddleHigh.getY()), PreLoadMiddleHigh.getHeading())
                .build();

                //Cone 1
        Trajectory conestack1 = drive.trajectoryBuilder(pLoadRedMid.end())
                .splineTo(new Vector2d(RightRedStack.getX(), RightRedStack.getY()), RightRedStack.getHeading())
                .build();
        Trajectory highpole1 = drive.trajectoryBuilder(conestack1.end(),true)
                .splineTo(new Vector2d(RightHighPole.getX(), RightHighPole.getY()), RightHighPole.getHeading())
                .build();
                //Cone 2
        Trajectory cone2 = drive.trajectoryBuilder(highpole1.end())
                .lineTo(RightRedStackV)
                .splineTo(new Vector2d(RightHighPole.getX(), RightHighPole.getY()), RightHighPole.getHeading())
                .build();
                //Parking Spots
        Trajectory parkspot1 = drive.trajectoryBuilder(cone2.end())
                .back(12)
                .build();
        Trajectory parkspot2 = drive.trajectoryBuilder(cone2.end())
                .forward(12)
                .build();
        Trajectory parkspot3 = drive.trajectoryBuilder(cone2.end())
                .lineTo(RightRedStackV)
                .build();
                /* ----------- Only using 1+2 for consistency currently 3/1/23 --------------------
                //Cone 3
                .lineTo(RightRedStackV)
                .splineTo(new Vector2d(RightHighPole.getX(), RightHighPole.getY()), RightHighPole.getHeading())
                //Cone 4
...
drive.followTrajectory(pLoadRedMid);
        sleep(2000);
drive.followTrajectory(conestack1);
        sleep(500);
drive.followTrajectory(highpole1);
        sleep(500);
drive.followTrajectory(conestack1);
        sleep(500);
drive.followTrajectory(highpole1);
        ...
        if(tagOfInterest == null)
        {
        drive.followTrajectory(parkspot2);
        }
        else
        {
            /*
